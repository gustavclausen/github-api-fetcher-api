...
describe('UsersController (e2e)', (): void => {
    ...
    describe('GET /:username', (): void => {
        ...
        describe('GET /contributions', (): void => {
            ...
            describe('GET /issues/month', (): void => {
                const sendRequest = async (
                    username: string,
                    year: number | null,
                    month: string | null
                ): Promise<request.Response> => {
                    return await buildGetRequest(app, `/users/${username}/contributions/issues/month`).query({
                        year,
                        month
                    });
                };
                ...
                it('should return 401 if no access token is passed', async (): Promise<void> => {
                    const response = await buildUnauthorizedGetRequest(
                        app,
                        '/users/dummy-user/contributions/issues/month'
                    );
                ...
                it('should return contributions if existing username is passed', async (): Promise<void> => {
                    const response = await sendRequest('gaearon', 2019, 'FEBRUARY');
                ...
                it('should return 400 if no year query parameter is passed', async (): Promise<void> => {
                    const response = await sendRequest('gaearon', null, 'JUNE'); // missing 'year' query parameter

                    expect(response.status).toBe(400);
                });

                it('should return 400 if invalid year query parameter is passed', async (): Promise<void> => {
                    const response = await sendRequest('gaearon', 1999, 'FEBRUARY'); // 'year' must be >= 2000

                    expect(response.status).toBe(400);
                });

                it('should return 400 if no month query parameter is passed', async (): Promise<void> => {
                    const response = await sendRequest('gaearon', 2019, null); // missing 'month' query parameter

                    expect(response.status).toBe(400);
                });

                it('should return 400 if invalid month query parameter is passed', async (): Promise<void> => {
                    const response = await sendRequest('gaearon', 2018, 'NON-EXISTING-MONTH'); // invalid 'month' query parameter

                    expect(response.status).toBe(400);
                });
            });

            describe('GET /issues/year', (): void => {
                const sendRequest = async (username: string, year: number | null): Promise<request.Response> => {
                    return await buildGetRequest(app, `/users/${username}/contributions/issues/year`).query({
                        year
                    });
                };
                ...
                it('should return 401 if no access token is passed', async (): Promise<void> => {
                    const response = await buildUnauthorizedGetRequest(
                        app,
                        '/users/dummy-user/contributions/issues/year'
                    );
                ...
                it('should return contributions if existing username is passed', async (): Promise<void> => {
                    const response = await sendRequest('gaearon', 2017);
                ...
                it('should return 400 if no year query parameter is passed', async (): Promise<void> => {
                    const response = await sendRequest('gaearon', null); // missing 'year' query parameter

                    expect(response.status).toBe(400);
                });

                it('should return 400 if invalid year query parameter is passed', async (): Promise<void> => {
                    const response = await sendRequest('gaearon', 1999); // 'year' must be >= 2000

                    expect(response.status).toBe(400);
                });
            });

            describe('GET /pull-requests/month', (): void => {
                const sendRequest = async (
                    username: string,
                    year: number | null,
                    month: string | null
                ): Promise<request.Response> => {
                    return await buildGetRequest(app, `/users/${username}/contributions/pull-requests/month`).query({
                        year,
                        month
                    });
                };
                ...
                it('should return 401 if no access token is passed', async (): Promise<void> => {
                    const response = await buildUnauthorizedGetRequest(
                        app,
                        '/users/dummy-user/contributions/pull-requests/month'
                    );
                ...
                it('should return contributions if existing username is passed', async (): Promise<void> => {
                    const response = await sendRequest('gaearon', 2019, 'JANUARY');
                ...
                it('should return 404 if non-existing username is passed', async (): Promise<void> => {
                    const nonExistingUsername = uuid();
                    const response = await sendRequest(nonExistingUsername, 2018, 'JANUARY');
            ...
            describe('GET /pull-requests/year', (): void => {
                const sendRequest = async (username: string, year: number | null): Promise<request.Response> => {
                    return await buildGetRequest(app, `/users/${username}/contributions/pull-requests/year`).query({
                        year
                    });
                };
                ...
                it('should return 401 if no access token is passed', async (): Promise<void> => {
                    const response = await buildUnauthorizedGetRequest(
                        app,
                        '/users/dummy-user/contributions/pull-requests/year'
                    );
                ...
                it('should return contributions if existing username is passed', async (): Promise<void> => {
                    const response = await sendRequest('gaearon', 2018);
            ...
            describe('GET /pull-request-reviews/month', (): void => {
                const sendRequest = async (
                    username: string,
                    year: number | null,
                    month: string | null
                ): Promise<request.Response> => {
                    return await buildGetRequest(
                        app,
                        `/users/${username}/contributions/pull-request-reviews/month`
                    ).query({
                        year,
                        month
                    });
                ...
                it('should return 401 if no access token is passed', async (): Promise<void> => {
                    const response = await buildUnauthorizedGetRequest(
                        app,
                        '/users/dummy-user/contributions/pull-request-reviews/month'
                    );
            ...
            describe('GET /pull-request-reviews/year', (): void => {
                const sendRequest = async (username: string, year: number | null): Promise<request.Response> => {
                    return await buildGetRequest(
                        app,
                        `/users/${username}/contributions/pull-request-reviews/year`
                    ).query({
                        year
                    });
                };
                ...
                it('should return 401 if no access token is passed', async (): Promise<void> => {
                    const response = await buildUnauthorizedGetRequest(
                        app,
                        '/users/dummy-user/contributions/pull-request-reviews/year'
                    );
